'use client';
// ============================================================
// COMPONENTE: ResultsPanel — Painel de Resultados v2
...
import type { ExtendedSimulationResults, ExtendedFuelResults } from '@/lib/logic/simulator';
...
interface ResultsPanelProps {
  results: ExtendedSimulationResults;
}
...
type SectionTab = 'ciclo' | 'combustao' | 'mecanica' | 'exaustao' | 'financeiro';

const SECTION_TABS: { id: SectionTab; label: string; icon: React.ReactNode }[] = [
  { id: 'ciclo', label: 'Ciclo Térmico', icon: <Gauge size={13} /> },
  { id: 'combustao', label: 'Combustão', icon: <FlaskConical size={13} /> },
  { id: 'mecanica', label: 'Mecânica', icon: <Wrench size={13} /> },
  { id: 'exaustao', label: 'Exaustão', icon: <Activity size={13} /> },
  { id: 'financeiro', label: 'Financeiro', icon: <TrendingUp size={13} /> },
];

export function ResultsPanel({ results }: ResultsPanelProps) {
  const [activeTab, setActiveTab] = useState<FuelId>('biogas');
  const fuelResult = results.fuels[activeTab] as ExtendedFuelResults | undefined;
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      {results.validation?.errors && results.validation.errors.length > 0 && (
        <NeuroCard className="p-4 bg-red-500/5 border border-red-500/20">
          <div className="flex gap-2">
      ...
      {/* Turbocharger badge */}
      {results.turbocharger && (
        <NeuroCard className="p-3 bg-blue-500/5 border border-blue-500/20">
          <p className="text-xs text-blue-300">
            Turbocompressor selecionado: <strong>{results.turbocharger.marca} {results.turbocharger.modelo}</strong>
            {' '}— RP [{results.turbocharger.rp_min}–{results.turbocharger.rp_max}], {results.turbocharger.potencia_kW} kW, {results.turbocharger.mancal}
          </p>
        </NeuroCard>
      )}
...
function FuelResultsView({ result }: { result: ExtendedFuelResults }) {
  const [activeSection, setActiveSection] = useState<SectionTab>('ciclo');
  const { cycle, chamber, mechanical, transmission, exhaust, financial, fuel } = result;
  const investimento = financial ? (80000) : 80000; // default, real value from params
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-4">
          ...
          <NeuroCard className="p-4">
            <h4 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3 flex items-center gap-2">
              <Gauge size={12} className="text-accent-blue" /> Diagrama T-s — Ciclo Brayton
            </h4>
            <TsDiagram cycle={cycle} fuelName={fuel.name} fuelColor={fuel.color} />
          </NeuroCard>

          {/* Estados do Ciclo */}
          <CycleStatesTable cycle={cycle} />
        </div>
      )}

      {/* ── TAB: COMBUSTÃO ── */}
      {activeSection === 'combustao' && (
        <div className="space-y-4">
          <div>
            <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
              <FlaskConical size={12} /> Consumo — {fuel.name}
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <MetricCard label="Vazão de Ar" value={cycle.m_ar_kgs.toFixed(4)} unit="kg/s" />
              <MetricCard label="Consumo Combust." value={result.consumo_kgh.toFixed(3)} unit="kg/h" />
              <MetricCard label="Consumo Volumét." value={result.consumo_m3h.toFixed(3)} unit="m³/h" />
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              />
              <MetricCard label="Diâmetro" value={chamber.d_camara_mm.toFixed(1)} unit="mm" />
              <MetricCard label="Comprimento" value={chamber.L_camara_mm.toFixed(1)} unit="mm" />
              <MetricCard label="Volume" value={chamber.V_camara_cm3.toFixed(1)} unit="cm³" />
      ...
      {activeSection === 'mecanica' && (
        <div className="space-y-4">
          <div>
            <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
              <Wrench size={12} /> Eixo e Torques
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <MetricCard label="Torque Turbina" value={mechanical.torque_turbina_Nm.toFixed(2)} unit="N·m" />
              <MetricCard label="Torque Gerador" value={mechanical.torque_gerador_Nm.toFixed(2)} unit="N·m" />
              <MetricCard label="Relação Transm." value={mechanical.relacao_transmissao.toFixed(1)} unit=":1" />
              {mechanical.shaft && (
                <>
                  <MetricCard label="Diâm. Mín. Eixo" value={mechanical.shaft.d_min_mm.toFixed(2)} unit="mm" />
                  <MetricCard
                    label="FS Goodman"
          ...
          <div>
            <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
              <Wrench size={12} /> Transmissão por Engrenagens
            </h3>
            <TransmissionPanel transmission={transmission} />
          </div>
        </div>
      )}

      {/* ── TAB: EXAUSTÃO / COGERAÇÃO ── */}
      {activeSection === 'exaustao' && (
        <div className="space-y-4">
          <div>
            <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
              <Activity size={12} /> Gases de Exaustão
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <MetricCard
                label="Temperatura Exaustão"
                value={(exhaust.T6_C).toFixed(1)}
                unit="°C"
                status={exhaust.T6_C > 400 ? 'ok' : 'warning'}
              />
              <MetricCard label="Vazão Mássica Exaustão" value={exhaust.m_exh_kgs.toFixed(4)} unit="kg/s" />
              <MetricCard label="Vazão Volumétrica" value={exhaust.V_exh_m3h.toFixed(1)} unit="m³/h" />
              <MetricCard
                label="Calor Disponível"
                value={exhaust.Q_exh_kW.toFixed(1)}
                unit="kW"
                status="ok"
                subtext="Potencial de cogeração"
              />
              <MetricCard
                label="Eficiência Global"
                value={exhaust.eta_global_pct.toFixed(1)}
                unit="%"
                status={exhaust.eta_global_pct > 60 ? 'ok' : exhaust.eta_global_pct > 40 ? 'warning' : 'error'}
                subtext="Com recuperação 60%"
              />
            </div>
          </div>
          <NeuroCard className="p-4">
            <h4 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-3">Composição dos Gases (% aprox.)</h4>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'CO₂', value: exhaust.composicao.CO2, color: 'text-red-400' },
                { label: 'H₂O', value: exhaust.composicao.H2O, color: 'text-blue-400' },
                { label: 'N₂', value: exhaust.composicao.N2, color: 'text-slate-400' },
                { label: 'O₂', value: exhaust.composicao.O2, color: 'text-green-400' },
              ].map(({ label, value, color }) => (
                <div key={label} className="bg-bg-input rounded-xl p-3 text-center">
      ...
      {/* ── TAB: FINANCEIRO ── */}
      {activeSection === 'financeiro' && financial && (
        <div className="space-y-4">
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-widest mb-1 flex items-center gap-2">
            <TrendingUp size={12} /> Análise Financeira — {fuel.name}
          </h3>
          <FinancialPanel
            financial={financial}
            fuelName={fuel.name}
...
function CycleStatesTable({ cycle }: { cycle: ExtendedFuelResults['cycle'] }) {
            ...
            <tbody>
              {states.map(({ n, desc, point }) => (
                <tr key={n} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-2 pr-3 text-right text-amber-300">{(point.T_K - 273.15).toFixed(2)}</td>
                  <td className="py-2 pr-3 text-right text-blue-300">{point.P_kPa.toFixed(3)}</td>
                  <td className="py-2 pr-3 text-right text-cyan-300">{point.h_kJkg.toFixed(3)}</td>
                  <td className="py-2 text-right text-green-300">{point.s_kJkgK.toFixed(5)}</td>
                </tr>
              ))}
            </tbody>
          </table>
